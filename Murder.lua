-- ============================================================================
-- 🔪 KILLER HUB | MURDER SUITE V8.2 (PROMETHEUS FIX & ADVANCED WALL-CHECK)
-- ============================================================================
...
local MurderTab = KillerHub:CreateTab("Murder", "rbxassetid://8097452837")
...
-- ============================================================================
-- 🧠 KNIFE BALLISTIC PREDICTION MOTOR (WALL-SAFE & SMOOTH)
-- ============================================================================
local function getAdvancedKnifePrediction(targetChar)
    ...
    -- Escala de altura para avatares pequeños / R15
    ...
    if physicsData then 
        smoothVelocity = physicsData.SmoothedVelocity 
    end

    if smoothVelocity.Magnitude < 0.15 then 
        return targetPosition, targetPosition 
    end
    ...
    -- Velocidad Horizontal & Límite de movimiento
    ...
    -- Detección de fintas / Cambios bruscos de dirección
    ...
    -- ==========================================
    -- 📐 PREDICCIÓN VERTICAL REBALANCEADA
    -- ==========================================
    ...
    if isAir then
        -- En el aire (Salto / Caída): Curva parabólica suave limitada
        local verticalVelocity = math.clamp(smoothVelocity.Y, -14, 18)
        local gravityCompensation = 0.5 * 196.2 * (travelTime * travelTime) * 0.08
        local rawYOffset = (verticalVelocity * (MurderConfig.VerticalPred * 4.5) * travelTime) - gravityCompensation
        
        -- Clamp estricto para evitar alturas/caídas desproporcionadas
        local clampedY = math.clamp(rawYOffset, -2.8, 3.2)
        verticalOffset = Vector3.new(0, clampedY, 0)
    elseif absYVelocity > 0.05 then
        -- En rampas o escaleras: Ajuste mínimo y suave
        local verticalVelocity = math.clamp(smoothVelocity.Y, -8, 8)
        local sliderScale = (MurderConfig.VerticalPred / 0.040)
        local rampY = verticalVelocity * travelTime * 0.35 * sliderScale
        verticalOffset = Vector3.new(0, math.clamp(rampY, -1.2, 1.2), 0)
    end

    -- ==========================================
    -- 🧱 SISTEMA ANTI-MUROS Y FINTAS EN CORREDORES
    -- ==========================================
    local wallParams = RaycastParams.new()
    wallParams.FilterType = Enum.RaycastFilterType.Exclude
    wallParams.FilterDescendantsInstances = {targetChar, LocalPlayer.Character, Camera}

    -- 1. Detección de colisión frontal en la dirección de carrera del objetivo
    if horizontalOffset.Magnitude > 0.1 then
        local moveRay = workspace:Raycast(targetPosition, horizontalOffset, wallParams)
        if moveRay and moveRay.Instance and moveRay.Instance.CanCollide then
            local distToWall = (moveRay.Position - targetPosition).Magnitude
            local safeDist = math.max(0, distToWall - 0.45)
            horizontalOffset = horizontalOffset.Unit * math.min(horizontalOffset.Magnitude, safeDist)
        end
    end

    local finalPredictedPos = targetPosition + horizontalOffset + verticalOffset

    -- 2. Raycast directo desde la posición actual del objetivo hacia la predicción
    local finalRay = workspace:Raycast(targetPosition, finalPredictedPos - targetPosition, wallParams)
    if finalRay and finalRay.Instance and finalRay.Instance.CanCollide then
        local hitDist = (finalRay.Position - targetPosition).Magnitude
        if hitDist > 0.4 then
            finalPredictedPos = targetPosition + (finalPredictedPos - targetPosition).Unit * (hitDist - 0.35)
        else
            finalPredictedPos = targetPosition
        end
    end

    -- 3. Protección de línea de visión del tirador (Evita disparar tras esquinas)
    local shooterRay = workspace:Raycast(localHrp.Position, finalPredictedPos - localHrp.Position, wallParams)
    if shooterRay and shooterRay.Instance and shooterRay.Instance.CanCollide then
        local shooterHitDist = (shooterRay.Position - localHrp.Position).Magnitude
        if shooterHitDist < (finalPredictedPos - localHrp.Position).Magnitude - 0.5 then
            finalPredictedPos = targetPosition
        end
    end
...
MurderTab:CreateToggle("KnifeAimActive", "Knife Thrown aim", function(state) 
    MurderConfig.SilentAim = state
    saveConfig() 
end)

MurderTab:CreateToggle("PrioritizeSheriffActive", "Prioritize Sheriff", function(state) 
    MurderConfig.PrioritizeSheriff = state
    saveConfig() 
end)

MurderTab:CreateToggle("KnifeWallCheckActive", "Wall Check", function(state) 
    MurderConfig.WallCheck = state
    saveConfig() 
end)

MurderTab:CreateSlider("KnifeHorizSlider", "Horizontal prediction", 0, 300, function(value) 
    MurderConfig.HorizontalPred = value / 1000
    saveConfig() 
end)

MurderTab:CreateSlider("KnifeVertSlider", "Vertical prediction", 0, 120, function(value) 
    MurderConfig.VerticalPred = value / 1000
    saveConfig() 
end)
...
MurderTab:CreateToggle("IncreaseHitboxMaster", "Increase Hitbox", function(state) 
    MurderConfig.HitboxEnabled = state
    saveConfig() 
end)

MurderTab:CreateToggle("SeeHitboxActive", "See hitbox", function(state) 
    MurderConfig.SeeHitbox = state
    saveConfig() 
end)

MurderTab:CreateSlider("HitboxSizeSlider", "Stab Hitbox Size", 2, 100, function(value) 
...
MurderTab:CreateToggle("ShowKnifePredictionVisual", "See prediction", function(state) 
    MurderConfig.ShowPredCircle = state
    saveConfig() 
end)

MurderTab:CreateToggle("SmartHandVisibility", "Smart Visibility", function(state) 
    MurderConfig.SmartVisibility = state
    saveConfig() 
end)

MurderTab:CreateSection("Field Of View (FOV)")

MurderTab:CreateToggleColorPicker("FovVisibleMurder", "FovColorMurder", "Show FOV Circle", MurderConfig.FOVColor, function(stateToggle) 
    MurderConfig.ShowFOV = stateToggle
    saveConfig() 
end, function(selectedColor) 
    MurderConfig.FOVColor = selectedColor
    saveConfig() 
end)

MurderTab:CreateSlider("FovRadiusMurder", "FOV Radius", 30, 600, function(value) 
    MurderConfig.FOVRadius = value
    saveConfig() 
end)
